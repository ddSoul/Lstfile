...
//                          };
//    
////    NSString *urlString = [NSString stringWithFormat:@"%@%@",serverUrl,orderListUrl];
//    NSString *urlString = @"http://172.16.6.91:8080/etc-card/service/v4/orders";
...
+ (void)getOrderListSuccess:(successHandler)success failure:(failureHandler)failure
{
    ...
    NSString *urlString = [NSString stringWithFormat:@"%@%@",serverUrl172,c_orderListUrl];
    [LSTNetWorkTool getWithUrl:urlString params:params success:^(id responseObject) {
        if (success) {
            success(responseObject);
        }
