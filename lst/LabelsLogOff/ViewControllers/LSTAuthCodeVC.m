...
- (void)verifyAuthCode
{
    /** 校验验证码的正确性*/
    self.active = NO;
//    [LSTEtagAPI vertifyAuthCodeWithCode:self.authCodeTf.text pwdtoken:self.pwdToken success:^(id result) {
//        //需要提交订单
//        [LSTEtagAPI postOrderWithOrder:self.orderModel carModel:self.carModel success:^(id result) {
...
//    
    LSTChangeLabelsDetailVC *changeLabelsDetailVC = [[LSTChangeLabelsDetailVC alloc] init];
    [self.navigationController pushViewController:changeLabelsDetailVC animated:YES];
...
- (void)senderAuthCode
{
    ...
    [LSTEtagAPI getPassWordTokenWithPhoneNumber:self.carModel.phone success:^(id result) {
   
        self.pwdToken = result[@"data"][@"passwordToken"];
        [LSTEtagAPI getAuthCodeWithPassWordToken:self.pwdToken success:^(id result) {
